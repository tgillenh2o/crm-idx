  ...
  return (
    <div className="container" style={{ maxWidth: 1100, margin: "24px auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
        <div>
          <strong>{user.name}</strong> <small style={{ color: "#6b7280" }}>({user.role})</small>
          <div style={{ color: "#6b7280" }}>{user.email}</div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn" onClick={() => { navigator.clipboard.writeText(localStorage.getItem("crm_token") || ""); alert("Token copied"); }}>Copy Token</button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 16 }}>
        ...
        <div>
          ...
          <div style={{ marginTop: 18 }}>
            <h3>Your Leads</h3>
            {user.role === "agent" ? <AgentLeads leads={leadsData} /> : <AgentLeads leads={leadsData} />}
          </div>
        </div>
      </div>
    </div>
