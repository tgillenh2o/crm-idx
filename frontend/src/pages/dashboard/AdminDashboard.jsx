...
export default function AdminDashboard() {
  ...
  // filter leads for lead pond vs assigned
      ...
      <div className={`main-panel ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
            ...
            {leadPondLeads.length > 0 && (
              <>
                ...
                <div className="leads-grid">
                  {leadPondLeads.map(l => (
                    <LeadCard key={l._id} lead={l} isAdmin onAssign={handleAssign} users={users} isLeadPond />
                  ))}
                </div>
              </>
            )}
        ...
        {activeTab === "my-leads" && otherLeads.length > 0 && (
          <>
            ...
            <div className="leads-grid">
              {otherLeads.map(l => (
                <LeadCard key={l._id} lead={l} isAdmin onAssign={handleAssign} users={users} onDelete={handleDelete} />
              ))}
            </div>
          </>
        )}
